...
// Static UI + health
// app.use(express.static(path.join(__dirname,'public'))); // Vercel handles this automatically from the project root
app.get('/healthz', (_req, res) => res.send('ok'));
// Remove wildcard handler that serves index.html, Vercel rewrites handle SPA fallbacks safely if configured, 
// or we can leave it but ensure it doesn't capture API routes.
// For Vercel, it is better to let configuration handle static files.
// app.get('*', (req,res,next)=>{ if(req.path.startsWith('/api/')) return next(); res.sendFile(path.join(__dirname,'public','index.html')); });

const PORT = process.env.PORT || 3000;
// Only listen if running locally (not in a Vercel serverless environment)
